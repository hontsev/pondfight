...
Pond.Duck.init = function() {
  ...
  var onresize = function(e) {
    ...
    tabDiv.style.left = rtl ? '100px' : '440px';
    tabDiv.style.width = (window.innerWidth - 500) + 'px';
    var divTop =
        Math.max(0, top + tabDiv.offsetHeight - window.pageYOffset) + 'px';
    var divLeft = rtl ? '100px' : '440px';
    var divWidth = (window.innerWidth - 500) + 'px';
...
Pond.Duck.changeTab = function(index) {
  ...
  if (Pond.isDocsVisible_) {
    ...
    frame.src = '/public/pondtest/docs.html?lang=' + BlocklyGames.LANG +
...
Pond.Duck.editorChanged = function() {
  ...
  if (Pond.Duck.blocksEnabled_) {
    if (!BlocklyGames.workspace.getTopBlocks(false).length ||true) {
